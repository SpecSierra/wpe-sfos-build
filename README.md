...
| `patches/webkit/webkit-jsc-shell-object-link.patch` | `keep temporarily` | fixes the JavaScriptCore `jsc` shell link on the Ubuntu 24.04 runner by replacing the broken custom `ar` archive path with a CMake static target linked under `--whole-archive` |
